<template>
  <div class="PageContainer">
    <el-breadcrumb separator-class="el-icon-arrow-right" class="Breadcrumbs">
      ...
      <el-breadcrumb-item :to="{ path: '/dismantles' }">{{
        $t('dismantles.title')
      }}</el-breadcrumb-item>
      <el-breadcrumb-item>{{ $t('dismantles.new') }}</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card shadow="always">
      <div slot="header" class="clearfix">
        <h2>{{ $t('dismantles.new') }}</h2>
      </div>
    ...
    async submitForm(formName) {
      ...
      await this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          await this.$store
            .dispatch('dismantles/ADD_DISMANTLE', {
