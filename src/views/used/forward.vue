    ...
    >
      <template #header>
        <div>
          <div class="left">
            ...
            <h2>{{ $t('actions.tariff_forward') }} <span> #{{ product.id }}</span></h2>
          </div>

          <p class="info_text">
            {{ $t('actions.tariff_forward_text') }}
          </p>
        </div>
      </template>
      ...
      <div>
        <el-radio-group v-model="chosenDay">
          <el-radio
            v-for="vip in forward_list"
            :key="vip.id"
            :label="vip.id"
          >
            {{ vip.service_period_in_days }} {{ $t('day') }} ( {{ $t('used.vip_service_label', { hours: vip.schedule_period_in_hours, times: vip.update_time }) }} ) - {{
              vip.price
            }} {{ vip.price_type.currency_code }}
          </el-radio>
        </el-radio-group>
      </div>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      forward_list: 'used/forward_list',
      loading: 'used/loading',
    }),
  },
  async beforeMount () {
    await this.GET_PRODUCT(this.$route.params.id)
    await this.GET_FORWARD()
  },
  methods: {
    ...mapActions({
      GET_PRODUCT: 'used/GET_PRODUCT',
      PAY_FORWARD: 'used/PAY_FORWARD',
      GET_FORWARD: 'used/GET_FORWARD',
    }),
    async save () {
      this.loading = true
      try {
    ...
    async pay () {
      await this.PAY_FORWARD({ partId: this.product.id, serviceId: this.chosenDay })
    }
  },
}
</script>
